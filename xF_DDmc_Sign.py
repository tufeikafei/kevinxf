...
def get_info(DD_token, DD_cookies):
    try:
        ...
        info_url = f' https://maicai.api.ddxq.mobi/user/info?api_version=9.7.3&app_client_id=3&app_version=2.9.0&app_client_name=activity&station_id=6110c52393bf0e000142e1e8&native_version=9.50.0&city_number=1001&device_token={DD_token}&device_id=570547e94815dbc72dbb6169cbe3c889&latitude=31.267509&longitude=120.611656'
        info_headers = {
            "user-agent": f"Mozilla/5.0 (Linux; Android 11; meizu 17 Pro Build/N2G47H; wv) AppleWebKit/537.36 (KHTML, like Gecko) Version/4.0 Chrome/81.0.4044.117 Mobile Safari/537.36 xzone/9.50.0 station_id/6110c52393bf0e000142e1e8",
...
def do_sign(name, DD_token, DD_cookies):
    try:
        ...
        data = f'api_version=9.7.3&app_version=2.9.0&app_client_id=3&station_id=6110c52393bf0e000142e1e8&native_version=9.33.0&city_number=1117&latitude=23.017158&longitude=113.811603'
        do_sign_heards = {
            "user-agent": f"Mozilla/5.0 (Linux; Android 11; meizu 17 Pro Build/N2G47H; wv) AppleWebKit/537.36 (KHTML, like Gecko) Version/4.0 Chrome/81.0.4044.117 Mobile Safari/537.36 xzone/9.50.0 station_id/6110c52393bf0e000142e1e8",
            "Cookie": DD_cookies,
        ...
        response = requests.post(
            url=do_sign_url, data=data, headers=do_sign_heards, verify=False)
        ...
        url = f'https://maicai.api.ddxq.mobi/point/home?api_version=9.7.3&app_client_id=3&app_version=2.9.0&app_client_name=activity&station_id=6110c52393bf0e000142e1e8&native_version=9.50.0&city_number=1001&device_token={DD_token}&device_id=570547e94815dbc72dbb6169cbe3c889&latitude=31.267509&longitude=120.611656&_={nowtime}'
        headers = {
            "user-agent": f"Mozilla/5.0 (Linux; Android 11; meizu 17 Pro Build/N2G47H; wv) AppleWebKit/537.36 (KHTML, like Gecko) Version/4.0 Chrome/81.0.4044.117 Mobile Safari/537.36 xzone/9.50.0 station_id/6110c52393bf0e000142e1e8",
            "Accept-Encoding": "gzip, deflate, br",
            "Connection": "keep-alive",
        ...
        # 查看积分有效期
        expire_point_display = result['data']['expire_point_display']

        success = result['data']['user_sign']['is_today_sign']  # 查看是否已签到
        # 查看连续签到多久
        sign_series = result['data']['user_sign']['sign_series']

        if success == True:
            # print("{1}成功签到，已连续签到{3}天，当前积分为：{0}分。".format(point_num,name2,expire_point_display,sign_series))
            msg("{3}成功签到,本次获得积分{4}，已连续签到{1}天，当前积分为：{0}积分。{2}".format(
                point_num, sign_series, expire_point_display, name, today_point))
            # if '成功签到' in msg_info:
            #     send("(叮咚买菜)",msg_info)
